 ...
 * Categoria: Feminino (id_category = 3)
 * id_tournament = 7
 *
 * NAO escreve nada no banco. Apenas le, calcula e salva JSON no scratchpad.
 *
 * Nota Nara Nunes (id_player 701): so joga >= 20:30.
 * A simulacao mexe apenas em ADVERSARIOS (pareamento), nao em horarios.
 * Portanto a restricao de horario nao afeta o algoritmo aqui;
 * mas listamos os jogos futuros da Nara no resultado para lembrar
 * de verificar o scheduled_at na hora de aplicar o novo pareamento.
 */
...
const OUT_FILE      = path.join(SCRATCHPAD, 'sim_fem.json');
...
/** Dois times podem se enfrentar: nenhum jogador em comum em nenhuma posicao */
...
/** Custo somente nos jogos futuros fornecidos (sem historico passado). */
...
function greedyMatching(duplas, hRight0, hLeft0) {
  ...
  let stuckCount = 0;
  const MAX_STUCK = unpaired.length * 2;
...
async function main() {
  console.log('=== SIM RE-OTIMIZACAO FEMININO (cat 3, torneio 7) ===\n');

  // 1. Rounds REGULAR da categoria 3 / torneio 7
  ...
  console.log(`Rounds REGULAR cat 3: ${roundIds.join(', ')}`);
  ...
  // Filtrar: ambas as duplas devem pertencer a rounds REGULAR cat 3
  ...
  console.log(`Matches cat 3: ${matches.length} | Passado: ${pastMatches.length} | Futuro: ${futureMatches.length}`);

  // 4. Historico passado (fixo, nao otimizavel)
  const hRight0 = {};
  const hLeft0  = {};
  ...
  if (futureDuplas.length < 2) {
    console.log('Menos de 2 duplas futuras -- nada a otimizar.');
    process.exit(0);
  }
  ...
  // Apenas duplas que aparecem nos futuros; leftover recebem 0 e falharia -- ok
  const cada1x = futureDuplas.every(d => {
    const cnt = countByDupla[d.id_double] || 0;
    return leftover.some(l => l.id_double === d.id_double) ? cnt === 0 : cnt === 1;
  });
  ...
  // 10. Nomes dos jogadores (de todos os matches, passados + futuros)
  ...
  // 11. Repeticoes remanescentes (passado + futuro otimizado)
  ...
  for (const [k, v] of Object.entries(totRFinal)) {
    if (v > 1) {
      ...
      repeticoesRemanescentes.push({
        lado:  'direita',
        par:   `${playerName[a] || a} vs ${playerName[b] || b}`,
        vezes: v,
      });
    }
  }
  ...
  // 14. Jogos da Nara (id_player 701) nos futuros otimizados
  //     Nota: a restricao >= 20:30 nao afeta o algoritmo de pareamento,
  //     mas listamos aqui para verificar scheduled_at na hora de aplicar.
  const jogosNara = [];
  for (const g of optimized) {
    const da = doublesMap[g.id_double_a];
    const db = doublesMap[g.id_double_b];
    const naraEmA = da && (da.right === ID_NARA || da.left === ID_NARA);
    const naraEmB = db && (db.right === ID_NARA || db.left === ID_NARA);
    if (naraEmA || naraEmB) {
      // Pega scheduled_at do match original correspondente (se houver)
      // No novo pareamento, os horarios serao redistribuidos -- mas listamos o que estava antes
      jogosNara.push({
        duplaA:      doublesMap[g.id_double_a]?.display_name || `double ${g.id_double_a}`,
        duplaB:      doublesMap[g.id_double_b]?.display_name || `double ${g.id_double_b}`,
        id_double_a: g.id_double_a,
        id_double_b: g.id_double_b,
        aviso:       'Nara Nunes (id 701) so joga >= 20:30 -- verificar scheduled_at ao aplicar',
      });
    }
  }

  if (jogosNara.length > 0) {
    console.log(`\nJogos da Nara Nunes (id 701) no novo pareamento: ${jogosNara.length}`);
    for (const j of jogosNara) {
      console.log(`  ${j.duplaA}  x  ${j.duplaB}`);
    }
  } else {
    console.log('\nNara Nunes nao aparece nos jogos futuros.');
  }
  ...
  const result = {
    ...
    n_jogos_otimizados: optimized.length,
    antes: {

...
| 10 | **Status `INJURED` — afastamento longo automático** | ⏸️ | Hoje atleta lesionado precisa marcar ausência semana a semana até segunda 18h (chato pra lesão de 4-6 semanas). Adicionar status `INJURED` em `players` (ou tabela `player_absences_long`) com `start_date` e `end_date` (opcional). `weeklyDrawService` exclui automaticamente do sorteio enquanto `today BETWEEN start AND end`. Atleta ou admin pode marcar/desmarcar via UI. Caso disparador: Maria Luísa (id 691, cat 3 Fem) rompeu panturrilha 28/04. Posição congela, sem perda de pontos. |

...
// Monta link wa.me a partir do whatsapp cadastrado (formatos variados:
// "51995491846", "51 99744-8695", "5198101212"). Tira tudo que não é dígito;
// se vier no formato nacional (<=11 dígitos, ex DDD+numero) prefixa 55.
// Mantém como está quando já tem código de país (12-13 dígitos).
const waLink = (whatsapp?: string): string | null => {
...
// Ícone do WhatsApp (lucide-react não traz ícones de marca)
                ...
                <tr key={player.id_player} className="group hover:bg-white/[0.02] transition-colors relative">
                    ...
                    <td className="px-6 py-6">
                        <div className="flex flex-col">
                            ...
                            {waLink(player.whatsapp) ? (
                                <a
                                    href={waLink(player.whatsapp)!}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    onClick={(e) => e.stopPropagation()}
                                    title={`Abrir conversa no WhatsApp com ${player.name}`}
                                    className="text-green-500 hover:text-green-400 text-xs font-bold font-mono flex items-center gap-1.5 w-fit transition-colors"
                                >
                                    <WhatsappIcon size={12} /> {player.whatsapp}
                                </a>
                            ) : (
